...
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install --no-install-recommends build-essential autotools-dev autoconf locales cmake git curl libcrypto++-dev libexiv2-dev libpq-dev libjpeg-dev liblcms2-dev libsigc++-2.0-dev libtiff5-dev ca-certificates ssl-cert -y
